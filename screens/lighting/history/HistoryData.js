...
import { getSensorValue, getSwitchValue, sendDataMobile, getLogHistory } from '../../../util/getPost';
...
function HistoryData(){
  ...
  const [dataLogHistory, setDataLogHistory] = useState({});
  ...
  const historyLogData = async () => {
    await getLogHistory()
    .then((res) => {
      const latest_doc = res.map((item) => item.latest_doc)
      console.log('log history ',latest_doc)
      setDataLogHistory(latest_doc)
    })
  }
  ...
  const Item = ({item}) => (
    <Card title={"Humidity Data"} style={[styles.shadow,{marginBottom: '5%'}]}>
    <View style={[styles.cardList]}>
      <View style={{paddingEnd: 20}}>
      <IconLabel icon={icons.phillipsBulb} label={""} />
      </View>
      <View>
    ...
    : 'N/A'}</Text>
      ...
      <View>
      {/* <Text style={[{color:'#060047'}]}>{item.lightStatus ? `TURN ON LIGHT` : 'TURN OFF LIGHT'}</Text> */}
    </View>
    <View style={{ width: '25%'}}>
