    ...
    await getSensorValue()
      .then((data) => {
        ...
        tenData.map(value => {
          ...
          last10Date.push(z);
          last10Dust.push(parseFloat(value.dustDensity));
        });

        // Dust data
        setTextSelected('Dust');
        setChartData({
          ...chartData,
  ...
  const getData = async () => {
    ...
    await getSensorValue()
      ...
      .then(data => {
        ...
        let z = y1[1] + ':' +y[1];
        last10Date.push(z);
        last10Dust.push(parseFloat(last_data.dustDensity)); // setTimeout(last10Date.push(last_data.datetime), 1000)
